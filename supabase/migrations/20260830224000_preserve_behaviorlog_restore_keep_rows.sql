-- Preserve accepted Keep rows while retaining the full graph for restore validation.
-- Exact preview, payload digest, ownership, and transaction guards remain enforced.
begin;
...
CREATE OR REPLACE FUNCTION public.apply_behaviorlog_restore_with_configuration_events(restore_payload jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
declare
  ...
  accepted_actions jsonb;
  kept_schedule_ids uuid[] := array[]::uuid[];
  previous_schedule_graphs jsonb := '{}'::jsonb;
...
begin
  ...
  select dry_run_summary -> 'actions' into accepted_actions
  from public.behaviorlog_import_runs
  where user_id = current_user_id
    and id = nullif(restore_payload ->> 'accepted_preview_run_id', '')::uuid
    and import_mode = 'restore_preview' and status = 'previewed';
  select coalesce(array_agg((action_row ->> 'localId')::uuid), array[]::uuid[])
  into kept_schedule_ids
  from jsonb_array_elements(coalesce(accepted_actions -> 'schedules', '[]'::jsonb)) action_row
  where action_row ->> 'action' = 'keep';
  ...
  if apply_run_status = 'previewed' then
    ...
    loop
      ...
      current_schedule_graph := cadence_private.current_behavior_schedule_graph(
        current_user_id, affected_behavior_id
      );
      previous_schedule_graphs := jsonb_set(previous_schedule_graphs,
        array[affected_behavior_id::text], current_schedule_graph, true);

      -- A metadata-only Behavior replacement must retain its kept schedule rows.
      -- An entirely kept graph also retains all parent identities and timestamps.
      if not exists (
        select 1 from jsonb_array_elements(coalesce(restore_payload -> 'schedules', '[]'::jsonb)) slot
        where (slot ->> 'behavior_id')::uuid = affected_behavior_id
          and not ((slot ->> 'id')::uuid = any(kept_schedule_ids))
      ) and not exists (
        select 1 from public.behavior_schedule_slots slot
        where slot.user_id = current_user_id and slot.behavior_id = affected_behavior_id
          and slot.id in (select value::uuid from jsonb_array_elements_text(
            coalesce(restore_payload -> 'delete_schedule_ids', '[]'::jsonb)))
      ) then
        ...
        select coalesce(
          jsonb_agg(
            jsonb_build_object(
              'id', (
                select existing_slot.behavior_schedule_id::text
                from public.behavior_schedule_slots existing_slot
                join jsonb_array_elements(coalesce(restore_payload -> 'schedules', '[]'::jsonb)) incoming
                  on (incoming ->> 'id')::uuid = existing_slot.id
                where existing_slot.user_id = current_user_id
                  and existing_slot.behavior_id = affected_behavior_id
                  and coalesce(incoming ->> 'parent_group_key', incoming ->> 'id') = grouped.parent_group_key
                order by existing_slot.sort_order, existing_slot.id limit 1
              ),
  ...
  foreach affected_behavior_id in array affected_behavior_ids
  loop
    if previous_schedule_graphs -> affected_behavior_id::text
        is distinct from derived_schedule_graphs -> affected_behavior_id::text then
