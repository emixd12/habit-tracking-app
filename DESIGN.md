...
- **Expanded details:** Native disclosure reveals description, category, schedule summary, and note directly on the row surface, without a grey panel, enclosing border, top divider, boxed card treatment, chevron, or separate disclosure icon. The detail block uses a small left inset. Measure spacing optically between rendered text and field edges: detail label/value spacing should visually match the Note label/textarea spacing, while spacing between detail pairs remains larger. While open, the whole occurrence row holds the same blue background used by that row's hover state. The Note textarea keeps its field border; Save note uses the same underlined text-action vocabulary as Completed and Not Completed.
