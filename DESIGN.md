...
- **Behavior records:** Active records are unboxed list rows separated by a single 1px Ash Line divider between adjacent records. They keep 20px padding, visible range-based adherence, resolved outcome counts, and a per-behavior calendar sized to the row. Lower-use behavior characteristics such as category, schedule, recurrence, reminders, and description live inside the row's Details and edit settings disclosure with the edit form. Create and edit forms use real field/control borders only; schedule slots use quiet row dividers instead of perimeter boxes.
...
- **Behavior review structure:** Overall adherence sits near the top of Behaviors with the active range selector beside it, and the overall calendar lives inside that same area. Review sections are unboxed report bands separated by single horizontal dividers where needed, not perimeter panel borders.
...
- **Behavior rows:** Behavior counts use unboxed divider rows with title, adherence label, MM-DD-YY tracking-since text, vertical Completed and Not Completed label/value counts with values left-aligned to the shared field column, and a seven-column heatmap sized to the row. Category, schedule, recurrence, reminders, and description sit inside Details and edit settings. Full completion is filled blue; partial states use factual diagonal marks; not completed states use Rust Signal without an added diagonal mark. The behavior tracking start date is marked with a dotted cell border when it falls inside the selected range. Non-empty behavior calendar cells open that behavior's Behavior date review area.
