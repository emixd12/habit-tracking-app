...
export function GoogleCalendarPanel({ coordinator, refreshRange, callbackMessage, wrongAccount = false, openExternalUrl, refreshVersion = 0 }: GoogleCalendarPanelProps) {
  ...
  const [calendars, setCalendars] = useState<readonly CalendarListEntry[]>([]);
  ...
  const save = async () => {
    ...
    try {
      ...
      await loadCalendars(coordinator, setCalendars, setMessage);
  ...
  const refresh = async () => {
    ...
    } catch { setMessage("Calendar refresh failed. Existing timeline context remains available when present."); }
    finally { setPending(null); }
  };
      ...
      <div className="grid max-w-2xl gap-4 text-sm leading-6">
        ...
        {canManage ? <>
          ...
          <fieldset className="grid gap-2 border-t border-line pt-4">
            ...
            {calendars.length === 0 ? <p className="text-muted-readable">Calendar list is not loaded yet.</p> : calendars.map((calendar) => <CalendarChoice key={calendar.id} calendar={calendar} preferences={preferences} onChange={setPreferences} />)}
          </fieldset>
          <div className="flex flex-wrap gap-3 border-t border-line pt-4">
            ...
            <button type="button" onClick={refresh} disabled={pending !== null || preferences.selectedCalendarIds.length === 0} className="product-action product-action-secondary min-h-11 px-3 py-2 text-sm font-bold">{pending === "refresh" ? "Refreshing Calendar…" : "Refresh Calendar"}</button>
...
function CalendarChoice({ calendar, preferences, onChange }: Readonly<{
  ...
  return <div className="grid gap-1 border border-line p-3">
    ...
    {selected ? <label className="flex min-h-11 items-center gap-2 text-muted-readable"><input type="checkbox" checked={!hidden} onChange={(event) => update(true, !event.target.checked)} />Visible on Timeline</label> : null}
  </div>;
}
...
async function loadCalendars(
  coordinator: GoogleCalendarCoordinator,
  setCalendars: (calendars: readonly CalendarListEntry[]) => void,
  setMessage: (message: string | null) => void,
): Promise<void> {
  try { setCalendars(await coordinator.listCalendars()); }
  catch { setMessage("Calendar list could not load."); }
}
