...
| Analytics calculations | `lib/resolvers/analytics.resolver.ts` | `lib/services/analytics.service.ts`, Analytics UI via service output, resolver tests | Components calculating adherence, components filtering behavior-day occurrence review rows, unresolved counted as failed completion, gamified streak logic | `docs/UI_SPEC.md`, `docs/PRODUCT_SPEC.md`, `docs/DATA_MODEL.md` | `tests/analytics.resolver.test.ts` | Unresolved is separate and excluded from final adherence unless docs change. Behavior-day review rows include all occurrence statuses for the selected behavior and local date so UI can reuse status and note services without recalculating the data contract. |
