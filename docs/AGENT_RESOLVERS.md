...
| BehaviorLog import validation and merge preview | `lib/resolvers/behaviorlog-import.resolver.ts` | `lib/services/behaviorlog-import.service.ts`, `lib/services/behaviorlog-import-write.service.ts`, resolver tests | Database writes inside the resolver, destructive restore/merge logic, API routes parsing JSONL or deciding conflicts, treating occurrence `current_status` as history, writing unsupported notes/interventions/reminder deliveries during preview or core merge apply, provider calls from imported intervention rows | `docs/EXPORT_FORMATS.md`, `docs/DATA_MODEL.md`, `docs/NOTIFICATION_SPEC.md` | `tests/behaviorlog-import.resolver.test.ts`, `tests/behaviorlog-import-merge-preview.test.ts`, `tests/behaviorlog-import-interventions.test.ts` | Import validation and merge preview are dry-run/plan-only; create-only and user-approved merge product writes are service/repository-owned and must consume a valid accepted preview. Merge preview emits deterministic `create_new`, `map_to_existing`, `skip_existing`, and `conflict_requires_decision` decisions plus stable conflict codes. Status history comes from `status_events.jsonl`, while `current_status` remains a snapshot. Optional notes are limited to occurrence-note safe-fill/conflict plans with sensitivity/source metadata; unsupported note targets and AI-generated notes are skipped. Optional interventions are parsed, hash/reference/channel/status validated, summarized by channel/status/linked behavior, and marked `preview_only`; they must not create reminder deliveries, schedule/send/cancel/retry reminders, or call Sequenzy/Web Push/provider APIs. Unknown core top-level fields are validation errors and app-specific fields must live under `extensions`. |
