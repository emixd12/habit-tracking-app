...
| `/login` | implemented | Google sign-in screen | Uses Supabase OAuth and returns through `/auth/callback`; links to Terms, Privacy, and Trust. During local non-production development, authenticated users can view it with `/login?preview=1`. |
...
| `/timeline` | implemented, protected | Primary screen for today's occurrences, a floating Needs decision modal for prior unresolved items plus same-day retained decisions, future preview, and the optional first-run setup pop-up | This is the main screen. First-run setup links into existing controls and can be dismissed locally. |
| `/behaviors` | implemented, protected | Primary behavior object surface for behavior settings, compact metadata, adherence, heatmaps, archived behavior access, and behavior date review for deliberate status and note corrections | First-run setup may link to `/behaviors#create-behavior`. Behavior date review reuses analytics resolver/service output and occurrence status/note services; it must not add schema, stored statuses, or date/time mutation. |
