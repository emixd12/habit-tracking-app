...
| `/timeline` | implemented, protected | Primary screen for today's occurrences, a floating Needs decision modal for prior unresolved items plus same-day retained decisions, future preview, inline occurrence time tracking, and the optional first-run setup pop-up | This is the main screen. Track Time is available for active current-day and visible Needs decision occurrences. Stop and Reset tracked time remain inside the existing occurrence disclosure; no route or API route is added. First-run setup links into existing controls and can be dismissed locally. |
