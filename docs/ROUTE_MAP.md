...
| `/timeline` | implemented, protected | Primary screen for today's occurrences, a floating Needs decision modal for prior unresolved items plus same-day retained decisions, future preview, and the optional first-run setup prompt | This is the main screen. First-run setup links into existing controls and can be dismissed locally. |
