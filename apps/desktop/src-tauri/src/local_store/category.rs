...
pub fn apply(db: &Connection, request: &Request) -> Result<Value> {
    ...
    let expected: HashMap<_, _> = expected_categories
        .iter()
        .map(|row| (row["id"].as_str().unwrap_or(""), row))
        .collect();
    if current.len() != expected_categories.len()
        || expected.len() != current.len()
        || next_categories.len() > 1000
    {
    ...
    for value in next_categories {
        ...
        let description = if value["description"].is_null() {
            ...
            Some(
                value["description"]
                    .as_str()
                    .ok_or("Invalid category description.")?
                    .to_string(),
            )
        };
        let sort_order = value["sort_order"]
            .as_i64()
            .ok_or("Category order is required.")?;
        ...
        if (!unchanged_name
            ...
            || description
                .as_ref()
                .is_some_and(|value| value.chars().count() > 2000 || value.contains('\0'))
            || sort_order < 0
    ...
    if affected.len() != updates.len() {
        return Err("Category assignments changed. Refresh Settings before deleting.".into());
    }
    for update in updates {
        let id = &update.graph.behavior.id;
        if !affected.contains(id.as_str())
            || !seen.insert(id)
            || update.graph.behavior.category_id.is_some()
        {
        ...
        behavior::write_graph(
            ...
            None,
            None,
            update.configuration_event.as_ref(),
        )?;
    }
    for id in removed {
    ...
    for row in next_rows {
        if let Some(previous) = current.iter().find(|old| old.id == row.id) {
            if previous.name == row.name
                ...
                && previous.sort_order == row.sort_order
            {
                continue;
            }
        }
